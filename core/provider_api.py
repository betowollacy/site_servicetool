...
_DURATION_RULES = [
    ...
    ('1m', ['1 mes', '1 mês', '1 month', '1 months', '1mes', '1meses']),
    ('3m', ['3 meses', '3 months', '3 month', '3meses', '3mes']),
    ('6m', ['6 meses', '6 months', '6 month', '6meses', '6mes']),
    ('12m', ['1 ano', '1 year', '12 meses', '12 months', '12 month', '1 yr', '1ano']),
    ('2y', ['2 anos', '2 years', '2 year', '2 ano', '2ano']),
    ('3y', ['3 anos', '3 years', '3 year', '3 ano', '3ano']),
]
...
def _match_acceptable(score, overlap, dur, rdur, kind, rkind, title='', remote_name=''):
    # Conflito duro: duracao ou tipo diferentes nunca integram.
    if dur and rdur and dur != rdur:
        return False
    if kind and rkind and kind != rkind:
        return False
    # A marca principal (nome da ferramenta) tem que estar no produto remoto.
    brand_tokens = _brand_tokens(title)
    if brand_tokens and brand_tokens[0] not in _norm_title(remote_name):
        return False
    # Aparelhos incompativeis (ex.: iPhone/iPad vs MacBook) nunca integram.
    lw, rw = set(_slug_words(title)), set(_slug_words(remote_name))
    if lw & {'iphone', 'ipad'} and rw & {'macbook', 'imac'}:
        return False
    if lw & {'macbook', 'imac'} and rw & {'iphone', 'ipad'}:
        return False
    if score >= 2.2:
        return True
    # Marca reconhecida + reforco (duracao ou tipo iguais) integra.
    if overlap >= 0.6 and ((dur and rdur and dur == rdur) or (kind and rkind and kind == rkind)):
        return True
...
def auto_link_service(service, min_score=2.2):
    ...
    if not _match_acceptable(score, overlap, dur, rdur, kind, rkind,
                             service.title, remote.SERVICENAME):
        return None, score
