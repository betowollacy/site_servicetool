...
class InventoryDeliveryTests(TestCase):
    ...
    def test_submit_local_order_stock_service_never_calls_api_when_empty(self):
        api = Api.objects.create(
            api_name='Provedor Teste', api_type='gsm', api_url='https://provedor.teste/public',
            api_username='user', api_key='KEY', status='Active',
        )
        self.service = ServiceList.objects.create(
            service_type='Server Service', service_group=self.group, title='AMT Aluguel 6h',
            original_price=Decimal('20.00'), status='Active', slug='amt-api-semestoque',
            api_enabled=True, api=api, referenceid='123',
        )
        inv = Inventory.objects.create(name='AMT API Vazio')
        self.service.inventory = inv
        self.service.save(update_fields=['inventory'])
        ...
        order.refresh_from_db()
        self.assertEqual(order.service_status, 'In Process')
        self.assertFalse((order.trx_id or '').strip())
...
class OrderEmailTests(TestCase):
    ...
    def test_split_creds_formats(self):
        ...
        self.assertEqual(notify._split_creds('user1:pass1'), ('user1', 'pass1'))
        self.assertEqual(notify._split_creds('user1;pass1'), ('user1', 'pass1'))
        self.assertEqual(notify._split_creds('852@tool<br>|senha1'), ('852@tool', 'senha1'))
        self.assertEqual(notify._split_creds('852@tool<br>'), (None, None))
        self.assertEqual(notify._split_creds('usuario sozinho'), (None, None))
...
class AdminServiceScreenshotUploadTests(TestCase):
    ...
    def test_edit_upload_screenshot_image_overwrites_url(self):
        ...
        buf = io.BytesIO()
        PILImage.new('RGB', (2000, 800), (30, 40, 50)).save(buf, format='PNG')
        buf.seek(0)
        img = SimpleUploadedFile('shot.png', buf.read(), content_type='image/png')
        self.client.force_login(self.staff)
        ...
        path = self.service.screenshot.replace('/media/', str(settings.MEDIA_ROOT) + os.sep).replace('/', os.sep)
        self.assertTrue(os.path.exists(path), path)
        with PILImage.open(path) as saved:
            self.assertLessEqual(saved.height, 460)
            self.assertLessEqual(saved.width, 1024)
